...
import {
  ...
  getAllParticipants,
  updateParticipant,
  downloadParticipantFormatted,
  getParticipantInfo,
} from "../helpers/ParticipantsApiHelper";
import {
  downloadExperimentDataFormatted,
  downloadSiteDataZip,
  fetchCombinedFileSize,
} from "../helpers/FilesAPIHelper";
...
const Admin: React.FC<AdminProps> = ({ match }) => {
  ...
  const [message, setMessage] = useState(
    "Note: Your account doesn't yet have an institutional agreement and/or IRB on file, and so is not appproved for human subjects research.",
  );
  const [loading, setLoading] = useState(true);
  const [alertColor, setAlertColor] = useState("#ff5959");
  let [showLogs, setShowLogs] = useState(null);
  let [participants, setParticipants] = useState<IParticipant[]>(
    prevValues ?? [],
  );
  ...
  useEffect(() => {
    if (experiment == undefined) {
      ...
      trackPromise(
        getExperiment(experimentId, auth.token, expActingFunction).catch(
          (err) => {
            if (err.status >= 500) {
              setError(
  ...
  useEffect(() => {
    if (experiment !== undefined) {
      const existingParticipantId = new URLSearchParams(location.search).get(
        "participantId",
      );
    ...
    getAllParticipants(auth.token, experimentId)
      .then((res) => {
        ...
        if (
          ...
          const site = experiment.sites.find(
            (site) => site.name === currentSite,
          );
          if (site) {
            res.participants = res.participants.filter(
              (participant) => participant.site._id === site._id,
            );
          }
        }
...
*/
  ...
  const { promiseInProgress: participantPromiseInProgress } = usePromiseTracker(
    { area: "participant" },
  );
  let setExcludeCallback = useCallback((uid, exclude) => {
    updateParticipant(uid, { exclude }, auth.token).then(async (res) => {
    ...
    async (uid) => {
      ...
      try {
        ...
        if (res && res.status === 404) {
          window.alert(
            "This participant has no associated data available to download.",
          );
        } else if (res && res.status >= 400) {
          window.alert(
            "An error occurred during download. Please refresh the page and try again.",
          );
        }
      } catch (error) {
    ...
    let mainColumns = [
      ...
      },
      {
        ...
        Cell: ({ row }) => {
          ...
          return (
            <p className={statusClass}>{ParticipantState[currentStatus]}</p>
          );
        },
      },
      {
                ...
                isDownloading ? (
                  ...
                  >
                    ...
                    <g mask="url(#mask0_538_2430)">
                      <path
                        d="M16 20L11 15L12.4 13.55L15 16.15V8H17V16.15L19.6 13.55L21 15L16 20ZM10 24C9.45 24 8.97917 23.8042 8.5875 23.4125C8.19583 23.0208 8 22.55 8 22V19H10V22H22V19H24V22C24 22.55 23.8042 23.0208 23.4125 23.4125C23.0208 23.8042 22.55 24 22 24H10Z"
                        fill="#816F7D"
                      />
                    </g>
                  </svg>
        ...
        Cell: ({ row }) => {
          ...
          useEffect(() => {
            const loadSize = async () => {
              const total = await fetchCombinedFileSize(
  ...
  let deleteParticipantCallback = useCallback(
    (participant) => {
          ...
          if (res.status === 200) {
            ...
            if (participants.length > 1) {
              setSelectedParticipant(
                participants[
                  (participantIndex + 1) % (participants.length - 1)
                ],
              );
            } else {
              setSelectedParticipant({
            ...
            setParticipants((prev) =>
              prev.filter((participant) => participant.uid !== participant.uid),
            );
          }
        })
        .catch((err) => {
  ...
  const onZipError = (err: any) => {
    ...
    if (err.toString().includes("404")) {
      setMessage("No recorded participant data found for this experiment");
    } else {
      setMessage(
        "An error occurred while downloading the experiment's data. Please refresh the page and try again.",
      );
    }
  ...
  return showLogs === null ? (
    <Container fluid className="container-fluid">
      ...
      />
      <ExperimentAccessError experimentId={experimentId} authToken={auth.token}>
        <HashRouter basename={`/`} hashType="noslash">
          <Container>
            <Container>
              <div className="experiment-results">
                ...
                <Row className="justify-content-between align-items-center gx-0">
                  ...
                  <Col xs={"auto"}>
                    <Row className="d-flex justify-content-center align-items-center">
                      <Button
                        className="tertiary-button me-3"
                        style={{
                          width: "160px",
                          color: "#644F64",
                          fontWeight: "bold",
                        }}
                        onClick={downloadExpData}
                      ...
                      {experiment && experiment.isMultiSite ? (
                        ...
                        >
                          ...
                          <Dropdown.Menu>
                            ...
                            {experiment ? (
                              experiment.sites.map((site, index) => {
                                return (
                                  <Dropdown.Item
                                    key={index}
                                    eventKey={site.name}
                ...
                <Button
                  ...
                  onClick={() => history.replace("/vera-portal/Dashboard")}
                >
                  Back
                </Button>
                <div style={{ height: "20px" }}></div>
            ...
            <Modal isOpen={show} toggle={() => setShow(!show)}>
              <ModalHeader>Create new participant</ModalHeader>
              <ModalBody></ModalBody>
              <ModalFooter>
            ...
            >
              ...
              <ModalBody>
                <p>
                  This will delete the participant and remove any related logs,
                  exit surveys and related data.
                </p>
                <Button
                  onClick={() => {
